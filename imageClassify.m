function [ M ] = imageClassify(filter,img,param)
...
for repNum = 1:imgClassify.nReps
    ...
    for i=1:img.nGroups
        ...
        % classify is doing the same thing as fitcdiscr  (pick one)
%         class = classify(subFilter(testId,:),subFilter(trainId,:), ...
%         img.group(trainId),imgClassify.Method);
    
        DAC =fitcdiscr(subFilter(trainId,:), img.group(trainId),'DiscrimType', imgClassify.Method);
        class=predict(DAC, subFilter(testId,:));


        for j=1:length(testId)            
            % strncmp--compare 1st n char of strings
            classNum = find(strncmp(class{j},img.groupList,length(img.group{i}))); 
